#!/usr/bin/env python3
"""
Entropy Calculator - Measures password randomness using Shannon entropy.
"""
import math
...
class EntropyCalculator:
    ...
    def _get_pool_size(self, password: str) -> int:
        pool = 0
    ...
    def _rating(self, bits: float) -> str:
        if bits >= 128:
            return 'Excellent (>=128 bits)'
        if bits >= 80:
            return 'Strong (80-127 bits)'
        if bits >= 60:
            return 'Good (60-79 bits)'
        if bits >= 40:
            return 'Fair (40-59 bits)'
        if bits >= 28:
            return 'Weak (28-39 bits)'
