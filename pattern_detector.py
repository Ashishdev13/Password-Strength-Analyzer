#!/usr/bin/env python3
"""
Pattern Detector - Identifies weak patterns in passwords.
"""
import re
...
class PatternDetector:
    ...
    SEQ_ALPHA = re.compile(
        r'(abcd|bcde|cdef|defg|efgh|fghi|ghij|hijk|ijkl|jklm|klmn|lmno|mnop|nopq|opqr|pqrs|qrst|rstu|stuv|tuvw|uvwx|vwxy|wxyz)',
        re.IGNORECASE,
    )

    def check_all(self, password: str) -> list[tuple[str, bool, str]]:
        return [
            ('keyboard_walk', not self._has_keyboard_walk(password),
             'No keyboard walk pattern (qwerty, asdf...)'),
            ('repeated_chars', not bool(self.REPEAT_PATTERN.search(password)),
             'No repeated characters (aaa, 111...)'),
            ('sequential_nums', not bool(self.SEQ_NUM.search(password)),
             'No sequential numbers (1234, 5678...)'),
            ('sequential_alpha', not bool(self.SEQ_ALPHA.search(password)),
             'No sequential letters (abcd, efgh...)'),
            ('date_pattern', not bool(self.DATE_PATTERN.search(password)),
             'No obvious date patterns (2024, 0101...)'),
            ('all_same_case', not self._all_same_case(password),
             'Mixed character cases used'),
        ]

    def _has_keyboard_walk(self, password: str) -> bool:
        lower = password.lower()
        return any(walk in lower for walk in self.KEYBOARD_WALKS)

    def _all_same_case(self, password: str) -> bool:
